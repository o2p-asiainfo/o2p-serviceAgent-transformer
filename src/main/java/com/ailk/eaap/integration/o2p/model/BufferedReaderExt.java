...
public class BufferedReaderExt extends Reader
{
    ...
    private void fill() throws IOException
    {
        ...
        if (dst < 0 || cb.length < 0 || (dst + cb.length) < 0)
        {
            ...
            bufferLoop: for (;;)
            {
                ...
                charLoop: for (i = nextChar; i < nChars; i++)
                {
                    ...
                    if ((c == split))
                    {
                        if (beSplit)
                        {
                            beSplit = beSplitBoolean(list, st, i, begin, vals);
                        }
                        else
                        {
                    ...
                    else if ((c == operator) || (c == operator2))
                    {
                        ...
                        if (beSplit)
                        {
                            beSplit = beSplitBoolean3(list, st, i, begin, vals);
                        }
                        else
                        {
                            String tmp = new String(cb, begin, i - begin);
    ...
    protected List<String> judgeList(List<String> list, StringBuffer st)
    {
    ...
    protected boolean beSplitBoolean3(List<String> list, StringBuffer st,
            int i, int begin, List<String> vals)
    {
        boolean beSplit;
        String str = new String(cb, begin, i - begin);
        if (!org.apache.commons.lang.StringUtils.isEmpty(str))
        {
            if (st.append(str) != null && list != null)
            {
                list.add(st.append(str).toString());
            }
    ...
    protected boolean beSplitBoolean(List<String> list, StringBuffer st, int i,
            int begin, List<String> vals)
    {
        boolean beSplit;
        if (list != null && list.size() > 0)
        {
     ...
     * 
     * @exception IllegalArgumentException
     *                If readAheadLimit is less than 0
     * @exception IOException
     *                If an I/O error occurs
     */
